...
def resolve_data_file() -> Path | None:
    """app.py와 같은 폴더에서 데이터 CSV를 찾는다.

    한글 파일명은 운영체제/깃 클라이언트에 따라 유니코드 정규화 방식(NFC/NFD)이 달라져
    화면상 이름은 같아도 바이트가 달라 정확히 일치하지 않는 경우가 있다. 이를 대비해
    정규화 비교로 한 번 더 찾고, 그래도 없으면 폴더 내 CSV가 1개뿐이면 그것을 사용한다.
    """
    exact = DATA_DIR / DATA_FILENAME
    if exact.exists():
        return exact
...
def main():
    ...
    if data_file is None:
        ...
        st.error(
            ...
            "app.py와 같은 폴더(리포 루트)에 CSV 파일이 커밋되어 있는지 확인해주세요.\n\n"
            f"현재 폴더에서 발견된 CSV 파일 목록: {found if found else '없음'}"
        )
        st.stop()
